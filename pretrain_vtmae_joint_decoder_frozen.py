...
def run_joint_training(
    *,
    ...
    for step in range(1, args.steps + 1):
        ...
        full_branch_loss = float(args.full_recon_weight) * full_recon_loss
        encoder_only_loss = (
        ...
        total_loss = full_branch_loss + encoder_only_loss

        encoder_opt.zero_grad(set_to_none=True)
        decoder_opt.zero_grad(set_to_none=True)

        retain_graph = bool(getattr(encoder_only_loss, "requires_grad", False))
        full_branch_loss.backward(retain_graph=retain_graph)
        if getattr(encoder_only_loss, "requires_grad", False):
            torch.autograd.backward([encoder_only_loss], inputs=encoder_params)
...
def main():
    p = argparse.ArgumentParser(
        description="Joint 3-camera training where decoder gradients are blocked on the single-camera reconstruction branch."
    )
    ...
    p.add_argument("--out", type=str, default="vtmae_joint_decoder_frozen.pt")
    ...
    print(f"[JOINT-DECODER-FROZEN-3CAM] channels={channels}, views={len(camera_names)}, size=({H},{W})", flush=True)
    print(f"[JOINT-DECODER-FROZEN-3CAM] cameras={camera_names}", flush=True)
    print(
        f"[JOINT-DECODER-FROZEN-3CAM] steps={args.steps} distill_mode={args.distill_mode} "
        f"single_recon={int(args.use_single_recon_loss)} warmup={args.distill_warmup_steps} "
        f"decoder_frozen_on_single_recon=1",
        flush=True,
    )
    ...
    run_joint_training(
        ...
        args=args,
        camera_names=camera_names,
        out_images_dir=args.preview_dir or f"joint_decoder_frozen_{Path(args.out).stem}_{'_'.join(camera_names)}_mw",
    )
...
python pretrain_vtmae_joint_decoder_frozen.py \
  ...
  --steps 100000 \
  --batch_size 32 \
  --out vtmae_joint_decoder_frozen.pt \
  --wandb \
  --wandb_project vtmae-joint-distill \
  --wandb_run joint_decoder_frozen
"""
